...
\begin{figure}[h!]
    \centering
    \begin{tikzpicture}
        \draw[-stealth] (-2,0) -- (6,0) node[below left] {$x$};
        \draw[-stealth] (0,-2) -- (0,5) node[below left] {$y$};
        \filldraw[cw2,opacity=0.1] (0,0) circle (1);
        \filldraw[cw2,opacity=0.1] (3,2) circle (2);
...
\begin{tikzpicture}
    \node at (0,3.75) {~};
    \filldraw[cw2,opacity=0.1] (0,0) circle (2);
...
\begin{tikzpicture}
    ...
    \filldraw[cw2,opacity=0.1] (x) circle (2.5);
...
\begin{prop}{}{Hausdorff}
    ...
    \tcblower
    \demostracion Sean $r = d(\mathbf{x}, \mathbf{y})$ y $s = r/2$, entonces las bolas $B(\mathbf{x}, s)$ y $B(\mathbf{y}, s)$ abiertas, tienen intersección vacía. En efecto, veamos que ningún punto de la primera puede estar en la segunda. Si $\mathbf{z} \in B(\mathbf{x}, s)$, entonces, por la desigualdad del triángulo
    \begin{align*}
...
\begin{tikzpicture}
    ...
    \filldraw[cw2,opacity=0.1] (a) circle (1.75);
    \filldraw[cw2,opacity=0.3] (b) circle (1.5);
